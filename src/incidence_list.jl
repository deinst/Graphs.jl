...
function inclist{V,E}(vs::Vector{V}, ::Type{E}; is_directed::Bool = true)  
    println("inclist constructor:", V)
...
vertex_index{V<:ProvidedVertexType}(g::GenericIncidenceList{V}, v::V) = vertex_index(v)
@deprecate vertex_index{V<:ProvidedVertexType}(v::V,g::GenericIncidenceList{V}) vertex_index(g,v)
edge_index{V,E}(e::E, g::GenericIncidenceList{V,E}) = edge_index(e)

out_edges{V}(v::V, g::GenericIncidenceList{V}) = g.inclist[vertex_index(g, v)]
out_degree{V}(v::V, g::GenericIncidenceList{V}) = length(out_edges(v, g))
out_neighbors{V}(v::V, g::GenericIncidenceList{V}) = TargetIterator(g, g.inclist[vertex_index(g, v)])
...
function add_edge!{V,E}(g::GenericIncidenceList{V,E}, u::V, v::V, e::E)
    ...
    ui::Int = vertex_index(g, u)
    push!(g.inclist[ui], e)
    g.nedges += 1

    if !g.is_directed
        vi::Int = vertex_index(g, v)
        push!(g.inclist[vi], revedge(e))
    end
end

function add_edge!{V,E}(g::GenericIncidenceList{V,E}, e::E)
    println("bang")
    s = source(g, e)
    t = target(g, e)
    add_edge!(g, s, t, e)
